...
def CompressOperator2(
    ...
    for solutionName in solutionNames:
        ...
        deltaCoefficients = np.zeros((numberOfSnapshots, numberOfModes))
        parameters = np.zeros((numberOfSnapshots, parameterDimension))
        ...
        for key, problemData in collectionProblemData.GetProblemDatas().items():
            ...
            times = solution.GetTimeSequenceFromCompressedSnapshots()
            previousTimes = np.append(times[0],times[:-1])


            localDeltaCoefficients = np.array([solution.GetCompressedSnapshotsAtTime(times[i])-solution.GetCompressedSnapshotsAtTime(previousTimes[i]) for i in range(len(times))])
            deltaCoefficients[count : count + localNumberOfSnapshots, :] = localDeltaCoefficients
        ...
        model, scalerParameter, scalerCoefficients = SLR.GridSearchCVRegression(regressor, paramGrid, parameters, deltaCoefficients)

        mins = np.min(deltaCoefficients, axis=0)
        maxs = np.max(deltaCoefficients, axis=0)

        operatorCompressionData[solutionName] = (model, scalerParameter, scalerCoefficients, mins, maxs)
