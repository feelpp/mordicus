...
def CheckIntegrity():
    ...
    timeIndices = np.array([0., 1., 2.5])
    vectors = np.array([np.ones(5), 2.*np.ones(5), 3.*np.ones(5)])
    vectorsDic = {}
    vectorsDic['vec1'] = np.ones(5)
    vectorsDic['vec2'] = 2.*np.ones(5)
    vectorsMap = ['vec1', 'vec2', 'vec1']
    
    TimeInterpolation(-1., timeIndices, vectors)
    TimeInterpolation(3., timeIndices, vectorsDic, vectorsMap)
    TimeInterpolation(1., timeIndices, vectors)
    TimeInterpolation(1., timeIndices, vectorsDic, vectorsMap)
    TimeInterpolation(0.4, timeIndices, vectors)
    TimeInterpolation(0.6, timeIndices, vectorsDic, vectorsMap)
    return "ok"
