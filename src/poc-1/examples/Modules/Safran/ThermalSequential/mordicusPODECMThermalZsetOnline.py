...
def test():
    ...
    snapshotCorrelationOperator = collectionProblemData.GetSnapshotCorrelationOperator("TP")
    reducedOrderBasisU = collectionProblemData.GetReducedOrderBasis("TP")
    ...
    import time
    start = time.time()
    onlineCompressedSolution = Th.ComputeOnline(onlineProblemData, timeSequence, reducedOrderBasisU, operatorCompressionData, 1.e-4)
    print(">>>> DURATION ONLINE =", time.time() - start)

    PW.WritePXDMF(mesh, onlineCompressedSolution, reducedOrderBasisU, "TP")
